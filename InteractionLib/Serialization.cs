...
namespace InteractionLib
{
    ...
    public class Serialization
    {
        ...
        /// <summary>
        /// The add <c>assambly</c> <c>plugin</c>.
        /// </summary>
        private static void AddAssamblyPlugin()
        {
            ...
            {
                string dirPlugin = AppDomain.CurrentDomain.BaseDirectory + "Plugins\\";

                AppDomain.CurrentDomain.AppendPrivatePath(dirPlugin + item.Attribute("Name").Value);
                Assembly asm = Assembly.LoadFrom(dirPlugin + item.Attribute("Assambly")?.Value);
